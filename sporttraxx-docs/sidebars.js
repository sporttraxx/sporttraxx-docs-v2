module.exports = {
  tracking: [
    {
      type: "category",
      label: "Event management",
      items: ["tracking/sop", "tracking/spi"],
    },
    {
      type: "category",
      label: "Pages",
      items: [
        "tracking/alarms",
        "tracking/checklist",
        "tracking/collect",
        "tracking/connections",
        "tracking/history",
        "tracking/issues",
        "tracking/manage",
        "tracking/map",
        "tracking/map2d",
        "tracking/order",
        "tracking/overview",
        "tracking/overview2",
        "tracking/speed",
        "tracking/stats",
        "tracking/timecheck",
        "tracking/tp",
        "tracking/trackanalysis",
        "tracking/trackanalysis2",
      ],
    },
  ],

  timing: [
    {
      type: "category",
      label: "Event management",
      items: [
        "timing/timing-before",
        "timing/timing-during",
        "timing/timing-after",
      ],
    },
    {
      type: "category",
      label: "Pages",
      items: ["timing/splits", "timing/timing"],
    },
  ],

  results: [
    {
      type: "category",
      label: "Event management",
      items: [
        "results/results-before",
        "results/results-during",
        "results/results-after",
      ],
    },
  ],

  hardware: [
    {
      type: "category",
      label: "Hardware",
      items: ["hardware/hardware-intro"],
    },
    {
      type: "category",
      label: "Tracking",
      items: [
        "hardware/gl200",
        "hardware/gl200bp",
        "hardware/gl200bt",
        "hardware/gl200uk",
        "hardware/gt200",
        "hardware/gl300",
        "hardware/gl310-320",
        "hardware/recce",
      ],
    },
    {
      type: "category",
      label: "Timing",
      items: ["hardware/clock", "hardware/starting-light"],
    },
    {
      type: "category",
      label: "Accessories and Tools",
      items: [
        "hardware/tools/30acharger",
        "hardware/tools/50acharger",
        "hardware/tools/craddle",
        "hardware/tools/gl200bptester",
        "hardware/tools/killer",
      ],
    },
  ],
};
